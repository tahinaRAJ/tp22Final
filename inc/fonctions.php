...
{
    $conn = dbconnect();
    $req = "SELECT e.emp_no, e.last_name, e.first_name FROM employees e JOIN dept_manager dm ON e.emp_no = dm.emp_no WHERE dm.to_date >= NOW() AND dm.from_date <= NOW() AND $current = d.dept_no";
    $result = mysqli_query($conn, $req);
    $managers = [];
    while ($row = mysqli_fetch_assoc($result)) {
        $managers[] = $row;
